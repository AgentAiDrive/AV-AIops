...
def _candidate_paths(yaml_path: str | None) -> List[Path]:
    ...
    p = Path(yaml_path)
    # If an absolute path was stored, prefer it directly
    if p.is_absolute():
        paths.append(p)

    # If a relative path was stored, try app recipes dir
    paths.append(_APP_RECIPES_DIR / p)

    # Also try user recipes dir by filename (import writes here)
...
def load_recipe_yaml_text(recipe: Recipe) -> str:
    ...
    # 3) Last chance: explicit override via env (comma-separated extra roots)
    extra_roots = os.getenv("AVOPS_RECIPES_DIRS", "")
    for root in [r.strip() for r in extra_roots.split(",") if r.strip()]:
        txt = _read_file_if_exists(Path(root) / Path(getattr(recipe, "yaml_path", "")).name)
        if txt is not None:
            return txt
...
# Optional helpers used by other pages (safe to keep)
