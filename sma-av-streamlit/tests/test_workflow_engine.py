...
from core.recipes import service as recipe_service
...
@pytest.fixture()
def sample_recipe(tmp_path, monkeypatch):
    monkeypatch.setattr(recipe_service, "RECIPES_DIR", str(tmp_path))
    recipe_path = tmp_path / "sample.yaml"
...
def test_execute_recipe_run_creates_evidence(db_session, sample_recipe):
    agent = Agent(name="Test Agent", domain="testing", config_json={})
    recipe = Recipe(name="Test Recipe", yaml_path=sample_recipe)
